...
/**
 * Class used to generate and store the balance between a given amount of
 * elements.
 * It generates a random balance using a matrix that has the following
 * requirements:
...
public class ElementsBalance {
    ...
    /**
     * Static method that returns a new instance of ElementsBalance with a random
     * damage table inside.
     * 
     * @param numberOfElements The size of the damage table.
     * @param maxDamage        Tells the algorithm to generate only values between
     *                         -maxDamage and maxDamage.
    ...
    public static ElementsBalance newRandomBalance(int numberOfElements, int maxDamage) {
        ...
        // Sets half the upper triangle of the matrix to values that allow the elements
        // to be balanced.
        for (int i = 0; i < (numberOfElements / 2); i++) {
            for (int j = i + 1; j < (numberOfElements / 2); j++) {
                int remaining = (numberOfElements / 2) - j;
                // Generates random damage within the bounds according to the parity of the
                // matrix.
            ...
            // Sets the last element of each iteration to a value that makes the sum of the
            // row zero.
            if (oddNumberOfElements) {
        ...
        // Adjusts the elements near the center of the matrix if the number of elements
        // in odd.
        if (oddNumberOfElements) {
            damageTable[numberOfElements / 2 - 1][numberOfElements / 2] = generateRandomDamage(maxDamage, 0, 1);
            damageTable[numberOfElements / 2 - 1][numberOfElements / 2
                    + 1] = -damageTable[numberOfElements / 2 - 1][numberOfElements / 2];
        } else {// Adjusts the elements near the center of the matrix if the number of elements
                // in even.
            damageTable[numberOfElements / 2 - 2][numberOfElements / 2] = generateRandomDamage(maxDamage / 2,
                    damageTable[numberOfElements / 2 - 2][numberOfElements / 2 - 1], 1);
            damageTable[numberOfElements / 2 - 1][numberOfElements
                    / 2] = damageTable[numberOfElements / 2 - 2][numberOfElements / 2 - 1]
                            - damageTable[numberOfElements / 2 - 2][numberOfElements / 2];
            damageTable[numberOfElements / 2 - 2][numberOfElements / 2
                    + 1] = -(damageTable[numberOfElements / 2 - 2][numberOfElements / 2 - 1]
                            + damageTable[numberOfElements / 2 - 2][numberOfElements / 2]);
        }

        // Fill the upper triangle by symmetrizing the matrix along the secondary
        // diagonal.
    ...
    private static int generateRandomDamage(int maxDamage, int partialSumOfRow, int remainingElementsInRow) {
        ...
        // If it is generating the second last value, it cannot be equal to the
        // -partialSumOfRow,
    ...
    // Returns the minimum safe number to generate a number valid to
    // generateRandomDamage requirements.
    private static int getLowerBoundNumberOfElements(int maxDamage, int partialSumOfRow, int remainingElementsInRow) {
        return Math.max(-maxDamage, -partialSumOfRow - remainingElementsInRow * maxDamage);
    }

    // Returns the minimum safe number to generate a number valid to
    // generateRandomDamage requirements.
    private static int getUpperBoundNumberOfElements(int maxDamage, int partialSumOfRow, int remainingElementsInRow) {
        return Math.min(maxDamage, -partialSumOfRow + remainingElementsInRow * maxDamage);
    }

    // Generates a random number between the specified bounds that is not equal to
    // zero or any given invalid values.
     ...
     * Can be negative if the second element is the strong element or zero if the
     * elements are equal
     */
