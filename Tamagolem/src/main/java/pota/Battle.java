...
public class Battle {
    ...
    public Battle(Player firstPlayer, Player secondPlayer, int numberOfElements, int golemHealth) {
        ...
        numberOfGolemsPerTeam = (int) Math
                .ceil((numberOfElements - 1.0) * (numberOfElements - 2.0) / (2.0 * numberOfStonesPerGolem));

        numberOfStonesPerElement = (int) Math
                .ceil(2.0 * numberOfGolemsPerTeam * numberOfStonesPerGolem / numberOfElements);
    ...
    /**
     * Starts the battle with the parameters passed in the constructor, it can be
     * called any amount of times
     * as it will generate a new balance every time and reset the Team of the two
     * Players, but the battle will have
     * always the same amount of elements and the maximum health of golems will be
     * the same.
     */
    public void start() {
        // Setup of the battle
    ...
    // Summons a golem for the player, checks that the golems of the two players
    // have different stones.
    private void summonGolem(Player summoner, Player other) {
        ...
        while (true) {
            ...
            // If the current golem is null it means that it is the first time a golem is
            // summoned in the battle.
    ...
    private Element[] retrieveStones(String nameSummoner) {
        do {
            ...
            for (int i = 0; i <= numberOfStonesPerGolem; i++) {
                ...
                // Prompts the user to confirm the elements he selected, eventually lets the
                // user select them again.
            ...
            // returns the previously selected stones before retrieving stones from the
            // beginning.
            returnStonesToStorage(stones);
        } while (true);
    }
    ...
    // Returns an element chosen by the user, checks if the stone wanted is in
    // storage,
    ...
    private Element readElement() {
        while (true) {
            try {
                String elementName = InputData.readNonEmptyString(Literals.INSERT_ELEMENT_NAME, true);
    ...
    // It adds the element stones of the golem passed as argument to the stone
    // storage.
    ...
    // Returns the balance as a table of strings with the names of the elements
    // also.
